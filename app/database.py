...
def match_user_id_with_note_id(note_id):
    ...
    _c = _conn.cursor()
    placeholder = "%s" if IS_POSTGRES else "?"
    _c.execute(f"SELECT user FROM notes WHERE note_id = {placeholder};", (note_id,))
    result = _c.fetchone()[0] if _c.fetchone() else None
    _conn.close()
    return result
...
def list_images_for_user(owner):
    ...
    _c = _conn.cursor()
    placeholder = "%s" if IS_POSTGRES else "?"
    _c.execute(f"SELECT uid, timestamp, name FROM images WHERE owner = {placeholder}", (owner,))
    result = _c.fetchall()
    _conn.close()
    return result
...
def match_user_id_with_image_uid(image_uid):
    ...
    _c = _conn.cursor()
    placeholder = "%s" if IS_POSTGRES else "?"
    _c.execute(f"SELECT owner FROM images WHERE uid = {placeholder};", (image_uid,))
    result = _c.fetchone()[0] if _c.fetchone() else None
    _conn.close()
    return result
