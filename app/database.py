...
def get_connection(db_type):
    if USE_POSTGRES:
        ...
        return sqlite3.connect(f"database_file/{db_type}.db")
...
def add_user(id, pw):
    _conn = get_connection("users")
    _c = _conn.cursor()
    _c.execute("INSERT INTO users values(%s, %s)" if USE_POSTGRES else "INSERT INTO users values(?, ?)", (id.upper(), hashlib.sha256(pw.encode()).hexdigest()))
    _conn.commit()
    _conn.close()
...
def write_note_into_db(id, note_to_write):
    ...
    _c.execute("INSERT INTO notes values(%s, %s, %s, %s)" if USE_POSTGRES else "INSERT INTO notes values(?, ?, ?, ?)", (id.upper(), current_timestamp, note_to_write, note_id))
    _conn.commit()
    _conn.close()
...
def image_upload_record(uid, owner, image_name, timestamp):
    _conn = get_connection("images")
    _c = _conn.cursor()
    _c.execute("INSERT INTO images VALUES (%s, %s, %s, %s)" if USE_POSTGRES else "INSERT INTO images VALUES (?, ?, ?, ?)", (uid, owner, image_name, timestamp))
    _conn.commit()
    _conn.close()
